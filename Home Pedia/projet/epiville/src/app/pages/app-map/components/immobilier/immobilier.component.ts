...
export class ImmobilierComponants implements OnInit {
  regions: any[] = [];
  departments: any[] = [];
  communes: any[] = [];
  ...
  ngOnInit(): void {
    this.http.get('/assets/updatedRegions.geojson').subscribe((data: any) => {
      ...
      const { minPrice, maxPrice } = this.averageCalcService.calculateMinMaxPrices(data.features);
      console.log('minPrice:', minPrice, 'maxPrice:', maxPrice); // Vérifiez les valeurs min et max
      const colorStops = this.colorService.generateColorStops(minPrice, maxPrice);
      console.log('colorStops:', colorStops); // Vérifiez les colorStops
      this.sharedDataService.setColorStops(colorStops);
      this.sharedDataService.setPropertyName('prix_m2_max');
      this.sharedDataService.setModeName('immobilier');
